...
class Loss:
    ...
    def compute_loss_kernel_grad(self, f):
        ...
        self.compute_grid_mass(f, 0)  # get the grid mass tensor...
        self.compute_sdf_loss_kernel.grad()
        self.compute_density_loss_kernel.grad()
        self.compute_grid_mass.grad(f, 0)  # back to the particles..
    ...
    def compute_loss(self, f):
        ...
        incremental_iou = max(min((loss_info['iou'] - self._init_iou) / (loss_info['target_iou'] - self._init_iou), 1),
                              0)
        loss_info['reward'] = r
