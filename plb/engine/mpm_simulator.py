...
class MPMSimulator:
    def __init__(self, cfg, primitives=()):
        ...
        self.np_dtype = np.float64 if cfg.dtype == 'float64' else np.float32
        ...
        if self.dim == 3:
            ...
            n_grid = self.n_grid = int(64 * quality)

        self.dx, self.inv_dx = 1 / n_grid, float(n_grid)
        self.dt = 0.5e-4 / quality * cfg.dt_quality
        if self.dim == 2:
        ...
        # material
        E, nu = cfg.E, cfg.nu  # E - young's modulus, Nu - poisson coefficient
        self._mu, self._lam = E / (2 * (1 + nu)), E * nu / ((1 + nu) * (1 - 2 * nu))  # Lame parameters

        self.mu = ti.field(dtype=dtype, shape=(max_particles,), needs_grad=False)
        self.lam = ti.field(dtype=dtype, shape=(max_particles,), needs_grad=False)
        self.yield_stress = ti.field(dtype=dtype, shape=(max_particles,), needs_grad=False)

        max_steps = self.max_steps = cfg.max_steps
        self.substeps = int(np.ceil(2e-3 / self.dt))


        self.x = ti.Vector.field(dim, dtype=dtype, shape=(max_steps, max_particles), needs_grad=True)  # position
        self.v = ti.Vector.field(dim, dtype=dtype, shape=(max_steps, max_particles), needs_grad=True)  # velocity
        self.C = ti.Matrix.field(dim, dim, dtype=dtype, shape=(max_steps, max_particles),
                                 needs_grad=True)  # affine velocity field
        self.F = ti.Matrix.field(dim, dim, dtype=dtype, shape=(max_steps, max_particles),
                                 needs_grad=True)  # deformation gradient

        self.F_tmp = ti.Matrix.field(dim, dim, dtype=dtype, shape=(max_particles),
                                     needs_grad=True)  # deformation gradient
        self.U = ti.Matrix.field(dim, dim, dtype=dtype, shape=(max_particles,), needs_grad=True)
        self.V = ti.Matrix.field(dim, dim, dtype=dtype, shape=(max_particles,), needs_grad=True)
        self.sig = ti.Matrix.field(dim, dim, dtype=dtype, shape=(max_particles,), needs_grad=True)

        self.grid_size = cfg.grid_size[:dim]
        res = tuple(int(np.ceil(i/self.dx)) for i in self.grid_size)

        self.res = res = res[:dim]
        ...
        self.object_id = ti.field(ti.i32, shape=(max_particles,))

        self.velocity_dumping = cfg.velocity_dumping
    ...
    def p2g(self, f: ti.i32):
        for p in range(0, self.n_particles):
            ...
            new_F = self.compute_von_mises(self.F_tmp[p], self.U[p], self.sig[p], self.V[p], self.yield_stress[p],
                                           self.mu[p])
            self.F[f + 1, p] = new_F
    ...
    def grid_op(self, f: ti.i32):
        for I in ti.grouped(self.grid_m):
            if self.grid_m[I] > 1e-12:  # No need for epsilon here, 1e-10 is to prevent potential numerical problems ..
                ...
                if ti.static(self.velocity_dumping > 0.):
                    v_out = v_out * self.velocity_dumping
    ...
    # ------------------------------------------------------------------
    @ti.func
    def compute_grid_m_helper(self, f, p):
        base = (self.x[f, p] * self.inv_dx - 0.5).cast(int)
        fx = self.x[f, p] * self.inv_dx - base.cast(self.dtype)
        w = [0.5 * (1.5 - fx) ** 2, 0.75 - (fx - 1) ** 2, 0.5 * (fx - 0.5) ** 2]
        for offset in ti.static(ti.grouped(self.stencil_range())):
